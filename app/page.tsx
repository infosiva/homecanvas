...
import {
  ...
  Briefcase, Palette, Layout, Check, X, Image,
} from 'lucide-react'
...
function StylePicker({ selected, onSelect }: { selected: string; onSelect: (id: string) => void }) {
  return (
    <div style={{ marginBottom: 16 }}>
      <label style={{ color: 'var(--ink-3)', fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', display: 'block', marginBottom: 8 }}>
        Choose your style
      </label>
        ...
        {DESIGN_STYLES.map(s => (
          <button
            ...
            style={{
              ...
              border: `1px solid ${selected === s.id ? 'rgba(139,92,246,0.7)' : 'var(--border-s)'}`,
              background: selected === s.id ? 'rgba(139,92,246,0.15)' : 'rgba(255,255,255,0.03)',
              color: selected === s.id ? '#a78bfa' : 'var(--ink-2)',
              cursor: 'pointer',
...
function BeforeAfterTeaser() {
  ...
  return (
    <motion.div
      ...
      style={{ maxWidth: 860, margin: '0 auto 60px', padding: '0 24px', position: 'relative', zIndex: 10 }}
    >
      <div style={{ textAlign: 'center', marginBottom: 20 }}>
        <span className="badge-aos" style={{ display: 'inline-flex', marginBottom: 12 }}>
          <Image size={11} />
          Before / After AI transformation
        </span>
        <h2 style={{ fontFamily: "'Outfit', sans-serif", fontWeight: 800, fontSize: 'clamp(22px, 3vw, 30px)', letterSpacing: '-0.03em', color: 'var(--ink-1)', margin: '0 0 6px' }}>
          See the difference in seconds
        </h2>
        <p style={{ color: 'var(--ink-2)', fontSize: 14, margin: 0 }}>Drag the slider to reveal the AI redesign</p>
      </div>

      <div
        className="card-aos"
        style={{ overflow: 'hidden', borderRadius: 16, position: 'relative', height: 280, cursor: 'col-resize', userSelect: 'none' }}
        onMouseMove={e => {
      ...
      >
        {/* Before */}
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg, #1a1625 0%, #2d1f3d 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 48, marginBottom: 8 }}>🛋️</div>
            <div style={{ color: 'var(--ink-3)', fontSize: 13, fontWeight: 600 }}>BEFORE</div>
            <div style={{ color: 'var(--ink-4)', fontSize: 12 }}>Original room</div>
          </div>
        </div>

        {/* After (clipped) */}
        <div style={{ position: 'absolute', inset: 0, clipPath: `inset(0 ${100 - pos}% 0 0)`, background: 'linear-gradient(135deg, #0f2027 0%, #203a43 40%, #2c5364 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'clip-path 0.05s linear' }}>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 48, marginBottom: 8 }}>✨</div>
            <div style={{ color: '#a78bfa', fontSize: 13, fontWeight: 700 }}>AFTER</div>
            <div style={{ color: 'var(--ink-3)', fontSize: 12 }}>AI redesign · Scandinavian</div>
          </div>
        </div>

        {/* Divider handle */}
        <div style={{ position: 'absolute', top: 0, bottom: 0, left: `${pos}%`, width: 2, background: '#a78bfa', transform: 'translateX(-50%)', pointerEvents: 'none' }}>
          <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 28, height: 28, borderRadius: '50%', background: '#a78bfa', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: '#fff', fontWeight: 800, boxShadow: '0 2px 12px rgba(139,92,246,0.6)' }}>⇔</div>
        </div>
      </div>
    </motion.div>
  )
    ...
    <>
      ...
      <section style={{ maxWidth: 860, margin: '0 auto', padding: '80px 24px 40px', textAlign: 'center', position: 'relative', zIndex: 10 }}>
        ...
        >
          ...
          <span style={{ background: 'linear-gradient(135deg, #8b5cf6 0%, #a78bfa 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
            in 30 seconds
          </span>
        </motion.h1>
        ...
        >
          Upload your room photo, pick your style, and watch AI transform it instantly. No design skills needed.
        </motion.p>
            ...
            {loading ? (
              ...
              <motion.form key="form" onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div>
                  ...
                  <input className="input" placeholder="e.g. My living room, 4x5m, north-facing" value={brand} onChange={e => setBrand(e.target.value)} required />
                </div>
                <div>
                  <label style={{ color: 'var(--ink-3)', fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', display: 'block', marginBottom: 6 }}>
                    Topic / angle
